...
public class PwalImpl implements Pwal, DeviceListener
{
	...
	// the Pwal Fully Qualified Domain Name
	private String fqdn;
	...
	public PwalImpl(List<DevicesManager> devicesManager)
	{
		...
		for (DevicesManager d : devicesManager)
		{
			...
			//patch do not start up to complete context start
	...
	@Override
	public void startManagers(boolean autostartOnly)
	{
		//iterate over available device managers
		for(DevicesManager d : this.devicesManagers.values())
		{
			//if autostart only check if the manager has the autostart flag
			if(autostartOnly)
			{
				if(d.isAutostart())
					this.startDeviceManager(d.getId());
