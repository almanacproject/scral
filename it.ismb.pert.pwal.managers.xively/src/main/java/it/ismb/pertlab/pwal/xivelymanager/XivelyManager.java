...
public class XivelyManager extends PollingDevicesManager<Datastream>
{
	...
	// private static final String[] TAGS = { "temperature", "humidity",
	// "light",
	...
	public XivelyManager(boolean autostart)
	{
		this.initCommon(autostart);	
	}
	
	private void initCommon(boolean autostart)
	{
		// build the poller
		...
		for (String tag : TAGS)
		{
			...
			for (int i = 1; i < N_OF_FEEDS_TO_SEARCH_PER_TIME && !stopSearch; i++)
			{
				...
				try
				{
					...
					{
						newMostRecentFeed = (!feedsToHandle.isEmpty()) ? feedsToHandle.get(0).getCreatedAt()
								: lastMostRecentFeed;
					}
				}
	 ...
	 */
	private void handleDatastream(Map<String, Object> mapOfStreams, Datastream stream, String tagToMatch)
	{
		...
		for (String key : mapOfStreams.keySet())
		{
			...
			if (key.contains("temperature"))
			{
				...
				this.addSubscription(
						new DataUpdateSubscription<Datastream>(60000, (ThermometerDevice) device, device.getId()));
			}
			else if (key.contains("humidity"))
			{
				device = new HumidityDevice(feedID, streamID);
				this.addSubscription(
						new DataUpdateSubscription<Datastream>(60000, (HumidityDevice) device, device.getId()));
			}
			else if (key.contains("light"))
			{
				device = new LightDevice(feedID, streamID);
				this.addSubscription(
						new DataUpdateSubscription<Datastream>(60000, (LightDevice) device, device.getId()));
			}
			else if (key.contains("pressure"))
			{
				device = new PressureDevice(feedID, streamID);
				this.addSubscription(
						new DataUpdateSubscription<Datastream>(60000, (PressureDevice) device, device.getId()));
			}
			else if (key.contains("fillLevel"))
			{
				device = new RiccardoFillLevel(feedID, streamID);
				this.addSubscription(
						new DataUpdateSubscription<Datastream>(60000, (RiccardoFillLevel) device, device.getId()));
			}
			if (device != null)
			{
	 ...
	 * 
	 * @return <code>true</code> if it is searching new feeds,
