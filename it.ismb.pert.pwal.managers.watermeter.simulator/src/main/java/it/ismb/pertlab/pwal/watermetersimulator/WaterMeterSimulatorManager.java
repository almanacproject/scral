...
public class WaterMeterSimulatorManager extends PollingDevicesManager<WaterMeterSensorData>
{
	...
	public WaterMeterSimulatorManager()
	{
		...
		this.initRandomCommon(WaterMeterSimulatorManager.DEFAULT_N_WATER_METERS, false);
	}
	
	/**
	 * Builds a water meter simulator for the gine number of meters.
	 * 
	 * @param nMeters
	 *            the number of meters to simulate
	 */
	public WaterMeterSimulatorManager(int nMeters)
	{
		this.initRandomCommon(nMeters, false);
	}
	 ...
	 *            the number of meters to simulate
	 * @param autostart
	 *            true if the manager should start automatically, false
	 *            otherwise
	 */
	public WaterMeterSimulatorManager(int nMeters, boolean autostart)
	{
		this.initRandomCommon(nMeters, autostart);
	}
	 ...
	 *            The polling time in milliseconds for getting measures out of
	 *            simulated water meters
	 */
	 ...
	 *            otherwise
	 */
	public WaterMeterSimulatorManager(int nMeters, int pollingTimeMillis, boolean autostart)
	{
		// set the polling time
		this.basePollingTimeMillis = pollingTimeMillis;
		this.requiredPollingTime = pollingTimeMillis;
	...
	private void initRandomCommon(int nMeters, boolean autostart)
	{
		...
		for (final SimulatedWaterMeter currentMeter : allMeters)
		{
			Callable<Boolean> task = new Callable<Boolean>() {
				...
				public Boolean call()
				{
					...
					addSubscription(new DataUpdateSubscription<WaterMeterSensorData>(
							Math.max(requiredPollingTime, WaterMeterSimulatorManager.MINIMUM_POLLING_TIME),
							currentMeter, currentMeter.getNetworkLevelId()));
