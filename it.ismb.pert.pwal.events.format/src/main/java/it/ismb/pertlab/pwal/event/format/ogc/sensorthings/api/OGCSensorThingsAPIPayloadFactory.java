...
public class OGCSensorThingsAPIPayloadFactory
{
	...
	public Set<Observation> getObservationDataPayload(PWALNewDataAvailableEvent event, String platformFQDN)
	{
		...
		// the device id
		String deviceId = event.getSender().getPwalId();// event.getSenderId();
		...
		for (String rawStreamName : values.keySet())
		{
			...
			dataStream.addObservation(observation);
			
			// set the result type
			// TODO: version 1 all events shall contain unitOfMeasures
			observation.setResultType(dataStreamName);
	...
	public Thing getSensorMetadataPayload(PWALBaseEvent event, String platformFQDN)
	{
		...
		return this.getSensorMetadataPayload(device, platformFQDN);
	}
	
	/**
	 * Translates the given device into an OGC SensorThings API compliant
	 * description to be exploited by calling modules
	 * 
	...
	public Thing getSensorMetadataPayload(Device device, String platformFQDN)
	{
		...
		// the device id
		String deviceId = device.getPwalId();
		...
		thingLocation.setId(uidGen.uidFromString(String.format("%.10f", device.getLocation().getLat()) + "-" + String.format("%.10f", device.getLocation().getLon())));
		...
		//thingLocation.addThing(thing);
		
		// build the datastreams descriptions
		this.generateDataStreamsFromDevice(device, deviceId, platformFQDN, thing);
	...
	/**
	 * Returns a device simple metadata definition, with only first-level ids
	 * are filled
	 * 
	 * @param device
	 *            The Pwal device instance to describe
	 * @param platformFQDN
	 *            The Pwal FQDN
	 * @return The simplified device description as OGC
	 */
	public Thing getSimpleSensorMetadata(Device device, String platformFQDN)
	{
		// create the UID generator
		UIDGenerator uidGen = UIDGenerator.getInstance();
		
		// the device id - unique
		// WAAAAAARNIIIIIIIIIIIINNNNNNNNGGGGGGGGGG!!!!
		// The pwal adopts a mixed phylosophy by which every device carries a
		// pwalId which is unique and an Id which somewhat identifies the
		// physical entity carrying the device. As such a single physical entity
		// might carry more than one device. In such a sense the entity is a
		// Thing in the OGC terminology. Unfortunately no means are provided to
		// access or list entities, therefore devices are treated as separated
		// entities.
		String deviceId = device.getPwalId();
	 ...
	 * @return
	 */
	private Set<Datastream> generateDataStreamsFromDevice(Device device, String deviceId, String platformFQDN, Thing thing)
	{
		...
		if(implementedInterfaces.length==0)
			//try getting the interfaces implemented by the superclass
			implementedInterfaces = device.getClass().getSuperclass().getInterfaces();
		
		// return the sensor name
		return this.generateDataStreamsFromImplementedInterfaces(implementedInterfaces, device, deviceId, platformFQDN, thing);
	}
	
	@SuppressWarnings("unchecked")
	private Set<Datastream> generateDataStreamsFromImplementedInterfaces(Class<?> interfaces[], Device device,
			String deviceId, String platformFQDN, Thing thing)
	{
			...
			if (Device.class.isAssignableFrom(interfaces[i]))
			{
				...
				if (methods.length > 0)
				{
					...
					for (Method m : methods)
					{
						...
						thing.addDatastream(dataStream);
				...
				else
				{
					...
					if (interfaces[i].isInterface())
					{
						...
						Set<Datastream> inheritedDatastreams = generateDataStreamsFromImplementedInterfaces(
								interfaces[i].getInterfaces(), device, deviceId, platformFQDN, thing);
	...
	private String generateThingClass(Device device)
	{
		...
		// check for the deviceOntClassMethod
		try
		{
			Method getOntClass = device.getClass().getMethod("getDeviceOntClass");
			
			if (getOntClass != null)
			{
				classURI = (String) getOntClass.invoke(device);
			}
		}
